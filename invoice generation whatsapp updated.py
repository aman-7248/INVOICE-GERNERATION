#import pywhatkit as kit
import mysql.connector
myfile=mysql.connector.connect(user='root',password='engineer@123',host='localhost',charset='utf8')
cursor=myfile.cursor()
cursor.execute("create database if not exists SHOP")
cursor.execute("use shop")
cursor.execute("create table if not exists customer(serial_no int(100)\
               ,customer_Name char(14) ,items_bought char(100),phone_no bigint(10),TOTAL_BILL int(30))")
#cursor.execute("drop table customer")
def whatsapp():
    from datetime import datetime
    userfile=open("invoice.txt","r")
    a=userfile.read()
    time = str(datetime.now())
    x=time[11:13]
    y=time[14:16]
    mobile=input("Enter customer mobile number to get E-bill:")
    mobile1="+91"+mobile
    kit.sendwhatmsg(mobile1,a,int(x),int(y)+2)
box=0
def login():
    while True:
        userfile=open("user.txt","r")
        a=userfile.read()
        s=a.split()
        user1=input("Enter username for login: ")
        if user1==s[0]:
            while True:
                pwd1=input("Input your password: ")
                if pwd1==s[1]:
                    print("You have succesfully loged in. Now access your account")
                    global box
                    box=1
                    return
                else:
                    print("wrong password!  Try again.")
        else:
            print("wrong username!  Try again.")

def setting():
        print()
        print()
        print()
        print("Choose an Option:")
        print("1). Edit Shop Name")
        print ("2). Delete Products")
        print("3). Change account Username and Password") 
        print("4). Back")
        choice = input()
        if choice == "1":
                print ("Enter Shop Name:")
                file = open("name1.txt","w+")
                file.write(input())
                file.close()
                print("Shop Name Changed!"+ "\n")
        elif choice == "2":
                print("Enter Product Code to be deleted: ")
                file = open("p_list","r")
                x = file.readlines()
                file.close()
                file = open("p_list","w+")
                new = []
                sk = input()
                for i in x:
                        if sk.upper() != strip(i):
                                new.append(i)
                file.writelines(new)
                file.close()
                print("Deleted Successfully!")
        elif choice=='3':
            while True:
                userfile=open("user.txt","r")
                a=userfile.read()
                s=a.split()
                user1=input("Enter your current username: ")
                if user1==s[0]:
                    while True:
                        pwd1=input("Enter your current password: ")
                        if pwd1==s[1]:
                            print()
                            userfile=open("user.txt","w+")
                            user=input("Enter new user name:")
                            pwd=input("Enter new password:")
                            userfile.write(user)
                            userfile.write("\n")
                            userfile.write(pwd)
                            userfile.write("\n")
                            userfile.close()
                            print ("Your account username and password is successfully changed.")
                            return
                        else:
                            print("wrong password. Try again.")
                
                else:
                    print("wrong username. Try again.")
                        
        elif choice == "4":
            return
def invoice():
        print()
        print()
        print()
        namec=input("input name of customer: ")
        phone=int(input("enter customer phone number: "))
        c = open ("invoice#.txt", "a")
        c.close()
        f = open("p_list","r")
        p_list = f.readlines()
        p1 = []
        for i in p_list:
                p1.append(strip(i))
        c = open("invoice#.txt","r")
        x = c.read()
        c.close()
        c = open("invoice#.txt","w")
        if len(x) == 0:
                c.write("1")
                counter = 1
        else:
                counter = int(x)
                counter = counter + 1
                c.write(str(counter))
                
        c.close()
        from datetime import datetime
        time = datetime.now()
        tim= str(time)
        timee = strip(strip(strip(strip(strip(strip(strip(tim)))))))
        file = open("name1.txt")
        name = file.read().upper()
        namep= ""
        for i in name :
                namep += i
                namep += " "
        ly = []
        ny = []
        a = 1
        print("Available Product Codes : ", end = "")
        m = 0
        for i in p1:
                m = m+1
                if m == len(p1):
                        print (i)
                else:
                        print(i,end =", ")
        while a ==1:
                print("Enter Product code:")
                p = input()
                if p.upper() not in p1:
                        print ("No Such Product Found!" + " Please Try Again")
                        continue
                ly.append(p.upper())
                print("Quantity:")
                while True:
                        try:
                                c = float((input()))
                                ny.append(c)
                                break
                        except:
                                print("Quantity can only be Integer or Float! Please Add Again")
                print ("Enter 1 to Stop adding product or press any other key to add more")
                if input() == "1":
                        print ("Enter Discount(in %)")
                        dis = float(input())
                        break
        p_list = []
        for i in range (len(ly)):
                p_list.append(ly[i])
        maxi = 0
        s_list = []
        a_list = []
        for i in p_list:
                x = ""
                for j in range (0,len(i)):
                        x = x + i[j]
                file = open(x+".txt")
                file.readline()
                s_list.append(len(file.readline()))
                a_list.append(x)
                file.close()
        x = open("invoice.txt","w+")
        if max(s_list) < 12:
                maxi = 12
        else:
                maxi = max(s_list)
        l = " " + "S.no" + " "*2+ "Product Name" + " "*(maxi-10) + "Price" + " "*4 + "GST(%)" + " "*2 +"Quantity" + "  "+ "Total"+" "
        f = len(l)
        z = int((f - len(namep))/2)
        v = int((f - len("INVOICE"))/2)
        w = int((f - len(timee))/2)
        s = int((f - 9 - len(str(counter)))/2)
        j = " "*z
        x.write(j)
        x.write(namep+"\n")
        x.write (" "*v + "INVOICE" + "\n"*2)
        x.write(" "*w + timee + "\n")
        x.write(" "*s + "Invoice#" + " " + str(counter)+"\n"*3)
        x.write((" " + "S.no" + " "*2+ "Product Name" + " "*(maxi-10) + "Price" + " "*4 + "GST(%)" + " "*2 +"Quantity" + "  "+ "Total"+" ")+"\n"+"\n")
        t = 0
        disc = 0
        z=''
        for i in p_list:
                t = t+1
                s_no = t
                code = i
                file = open(i+".txt","r")
                code = strip(file.readline())
                
                name = strip(file.readline())
                z+=name
                z+=','
               
                price = int(strip(file.readline()))
                gst = int(strip(file.read()))
                qt = (ny[t-1])
                x.write((" " + str(s_no) + ")." + " "*(4-len(str(s_no))) + name + " "*(maxi +2-len(name)) + str(price) + " "*(9-len(str(price))) + str(gst) + " "* (8-len(str(gst))) + str(qt) + " "* (10 - len(str(qt))) + str(round((price*float(qt)+price*gst*int(qt)/100),2)) + "\n"))
                dc = price*float(qt)+price*gst*float(qt)/100
                disc = disc + dc
        d = disc*dis*0.01
        x.write("\n")
        x.write(" "*len(" " + "S.no" + " "*2+ "Product Name" + " "*(maxi-10) + "Price" + " "*4 + "GST(%)" + " ") +" "+ "Discount:" + " " + str(round(d,2))+"\n")
        x.write(" "*len(" " + "S.no" + " "*2+ "Product Name" + " "*(maxi-10) + "Price" + " "*4 + "GST(%)" + " ") +" "+ "Net Payable:" + " " + str(round(disc-d,2))+"\n"*4)
        x.write ("*. This is a Computer Generated Invoice and does not require Signature."+"\n")
        x.write("*. Return/Replacement is not allowed under any circumstances."+"\n")
        x.write("\n"+"Thanks for shopping with us")
        file.close()
        x.close()
        print("Invoice Saved Successfully in Invoice.txt at Desktop.")
        print("Net Payable : ",round((disc-d),2))
        total=round((disc-d),2)
        #am=
        #for i in range(0,len(ly)):
            #am+=ly[i]
            #am+=","
        
        cursor.execute("insert into customer(serial_no,customer_Name,items_bought,phone_no,TOTAL_BILL)\
               values({},'{}','{}',{},{})".format(counter,namec,z,phone,total))
        myfile.commit()
        print("Press Any Key to continue!")
        input()
def strip(a):
        x = ""
        for i in range(0,len(a)-1):
                        x = x + a[i]
        return(x)
def add():
        print()
        print()
        print()
        name = input("Enter name of Product: ")
        while(True):
                try:
                        s = int(input("Enter Selling Price(Excluding GST): "))
                        g = int(input("Enter GST (in %): "))
                        n1 = input("Choose a Product code(Any alphanumeric character(Except spaces) to be used for Product Indentification): ")
                        le = open("p_list","r")
                        if n1.upper()+"\n" in le.readlines():
                                print("Product Already Exist!")
                                le.close()
                                return
                        break
                except:
                        print("Only Numerical Values are allowed in Selling Price and GST")
        f_name = n1.upper()+".txt"
        file = open(f_name,"w+")
        w = n1.upper() + "\n" + name + "\n" + str(s) + "\n"+ str(g) + "\n"
        file.write(w)
        file.close()
        file = open("p_list","a+")
        file.write(n1.upper()+"\n")
        file.close()
        print("")
        print("Product added Successfully!")
        print("")
        print ("Press any key to Continue to Main Menu")
        if (len(input())!= 0):
                return
def about():
        print("\n\n\n\n\t\t\tABOUT THE PROGRAM\n\n")
        print("NOWADAYS A LOT OF PROBLEMS HAVE SEEN FOR THE SHOPKEEPER\
 REGARDING THE BILL OF TRANSACTION MADE .\nWE HAVE CREATED THIS PROGRAM TO HELP THE SHOPKEEPER TO CREATE\
 A PROPER BILL OF THE TRANSACTION MADE BY THE CUSTOMER.\
\nTHIS PROGRAM AIMS AT MAINTAINING PROPER RECORDS OF\
 TRANSACTIONS MADE BY THE CUSTOMER IN A DATABASE SOFTWARE.\n\n\n\n\
CREATED BY : AMAN\n\
CLASS 12\n\
BATCH: 2020-2021\n\n\n\n\n")
def view():
        print()
        print()
        print()
        maxi = 0
        file = open("p_list","r")
        if len(file.read()) == 0:
                print("Product List Empty!")
                print("Press any key to continue!")
                input()
                return
        file.close()
        file = open("p_list","r")
        p_list = file.readlines()
        s_list = []
        a_list = []
        for i in p_list:
                x = ""
                for j in range (0,len(i)-1):
                        x = x + i[j]
                file = open(x+".txt")
                file.readline()
                s_list.append(len(file.readline()))
                a_list.append(x)
                file.close()
        if max(s_list) < 12:
                maxi = 12
        else:
                maxi = max(s_list)
        print(" " + "S.no" + " "*2+ "Product Name" + " "*(maxi-10) + "Price" + " "*4 + "GST(%)" + " "*2 + "Product Code"+" ")
        t = 0
        for i in a_list:
                t = t+1
                s_no = t
                code = i
                file = open(i+".txt","r")
                code = strip(file.readline())
                name = strip(file.readline())
                price = strip(file.readline())
                gst = strip(file.read())
                print(" " + str(s_no)  + ")." + " "*(4-len(str(s_no))) + name + " "*(maxi +2-len(name)) + price + " "*(9-len(str(price))) + gst ,end ="")
                g = 8 - len(str(gst))
                print(" "*g + code)
        print("")
        print ("Press any key to Continue to Main Menu")
        if (len(input())!= 0):
                return
def transaction():
    print()
    print()
    print()
    cursor.execute("select * from customer")
    d=cursor.fetchall()
    if d==[]:
        print("no transactions made till now")
    else:
        for i in d:
            print(i[0],' ',
                  'Customer Name:',i[1],'    ',
                  ' Phone no: ',i[3],' Amount: ',i[4],
                  '    ','Product sold: ',i[2])
def change():
    print()
    print()
    print()
    z=1
    pc=input('Enter the product code : ')
    pcf=pc.upper()+'.txt'
    c=open(pcf,'r+')
    while z==1:
        z=0
        print('Select an option to change that detail ')
        #print('1) Change product code')
        print('1) Change selling price')
        print('2) Change GST%')
        while(True):
            try:
                choice = int(input("Your Choice : "))
                break
            except:
                print("Choose correct Option! ")
        '''if choice==1:
            a=c.readlines()
            pc=a[0]
            print('Current product code',pc)
            c.close()
            npc=input('Enter new product code: ')
            c=open(pcf,'w')
            a[0]=npc+'\n'
            c.writelines(a)
            c.close()
            print('Detais successfully changed')
            return'''
        if choice==1:
            a=c.readlines()
            sp=a[2]
            print('Current selling price',sp)
            c.close()
            nsp=input('Enter new selling price: ')
            c=open(pcf,'w')
            a[2]=nsp+'\n'
            c.writelines(a)
            c.close()
            print('Detais successfully changed')
            return
        elif choice==2:
            a=c.readlines()
            gst=a[3]
            print('Current GST(in %) ',gst)
            c.close()
            ngst=input('Enter new GST(in %): ')
            c=open(pcf,'w')
            a[3]=ngst+'\n'
            c.writelines(a)
            c.close()
            print('Detais successfully changed')
            return
        else:
            z=1
            print('Enter correct option')
            print()
def customer_details():
    mobile=int(input("enter customer mobile number"))
    cursor.execute("select * from customer where phone_no={}".format(mobile))
    st=cursor.fetchall()
    if st==[]:
        print("no records exist with this number")
    else:
        for i in st:
            print("serial number:",i[0])
            print("name of customer:",i[1])
            print("items bought:",i[2])
            print("phone number:",i[3])
            print("total bill paid :",i[4],"rs")
    
    
userfile=open("user.txt","a")
userfile.close()
userfile=open("user.txt","r+")

leng=userfile.read()
if len(leng)==0:
    print("\t\t\t\tCREATE YOUR ACCOUNT")
    userfile=open("user.txt","a+")
    user=input("enter the user name:")
    pwd=input("input your password:")
    userfile.write(user)
    userfile.write("\n")
    userfile.write(pwd)
    userfile.write("\n")
    userfile.close()
if len(leng)>0:
        login()
if box==1:
    file = open("p_list","a+")
    file.close()
    file = open("name1.txt","a")
    file.close()
    file = open("name1.txt","r")
    s = file.read()
    file.close()
    if len(s) == 0:
        s_name = str(input("Enter name of Shop :"))
        file = open("name1.txt","w")
        file.write(s_name)
        file.close()
    file = open("name1.txt","r")
    k = file.read()
    j = "  "
    for i in k:
        j = j+i
        j = j + "  "
    print ("\n\n\n\n\t\t\t\t","|"+j.upper()+"|")
    for i in range (3):
        print("")
    while True:
        print()
        print()
        print()
        print()
        print("Select an Option:")
        print("1). Generate Invoice")
        print ("2). Show my Till Date Transactions")
        print("3). Add Products")
        print ("4). Veiw Products")
        print("5). Edit product details")
        print ("6). Settings")
        print ("7). customer details")
        print ("8). About Program")
        #print("9). Get Electronic bill on whatsapp")
        print ("9). Exit")
                
        while(True):
            try:
                choice = int(input("Your Choice : "))
                break
            except:
                print("Choose correct Option! ")
        if choice ==3:
            add()
        elif choice == 4:
            view()
        elif choice == 1:
            invoice()
        elif choice==5:
            change()
        elif choice ==10:
            print ("Program Closed!")
            break
        elif choice == 6:
            setting()
        elif choice==8:
            about()
        elif choice==2:
            transaction()
        elif choice==7:
            customer_details()
        #elif choice==11:
            #whatsapp()
        else:
            print("Choose Correct Option!"+"\n")
